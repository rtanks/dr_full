...
export default function PatientEdit({patient, onClick}) {
    ...
    //     console.log("first")
    // },[])
    const {editUserMutation} = patientService();

    const onSubmit = (data) => {
        console.log(data)
        const {aboutMe, ...mainData} = data;
        console.log(mainData);
        editUserMutation.mutate({id: patient._id, data: mainData})
    }
    return (
        <ContainerDrawer onClick={onClick} header={"ویرایش کاربر"}>
            <form onSubmit={handleSubmit(onSubmit)} className="w-[90%] h-max mx-auto flex flex-col gap-6 mb-5">
                ...
                <SubmitButtonFixed title={"ثبت اطلاعات"} title2={'در حال ثبت اطلاعات...'} isSubmitting={editUserMutation.isPending} additionalStyle={"bg-[#E9E9E9] text-[#676767] border-none"} disabled={false}/>
            </form>
        </ContainerDrawer>
    )
}