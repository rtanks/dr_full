...
export default function RecordDoctor() {
  ...
  // const {isLoading} = useQuery({queryKey: ["doctors"], queryFn: getDoctorsList})
              ...
              <div className="w-full h-max flex flex-col gap-2">
                {/* {
                  doctors?.map((doctor,index) => (
                    <RecordDoctorItem key={doctor.id} doctor={doctor} name={`${index + 1}-${doctor.user_FullName}`} number={"415241"} expertise={doctor.degree} phoneNumber={doctor.user_PhoneNumber} province={"لرستان"} city={doctor.city} status={doctor.isActive ? "فعال" : "غیر فعال"} getItemSelected={getItemSelected}/>
                  ))
                } */}
              </div>
            </div>
