...
const Calender = ({
  ...
  useEffect(() => {
    data ? setYears(calculatePersianAgeFromDate(data)) : setYears(null);
  ...
  return (
    <>
      ...
      <div className={` ${style} child:bg-white top-0 right-0 bottom-0 left-0 flex flex-col mx-auto my-auto h-fit gap-1 overflow-hidden relative md:right-0 md:left-0 `}>
        <p className="start-1 -top-3 px-2 text-sm">{title}</p>
        <div
          className={`text-middle relative mx-auto h-12 w-full ${
            alert && "!mb-13"
          } flex cursor-pointer absolute top-0 right-0 left-0 mx-auto w-full items-center justify-between border-b px-4 font-semibold text-gray-500 rounded-xl border border-gray-300 bg-white placeholder-gray-400 outline-gray-300 focus:placeholder-transparent dark:bg-white placeholder`}
          onClick={() => {
        ...
        >
          <p
            ...
            } w-fit rounded-lg px-2 pt-0.5 text-black`}
          >
            {String(data || "انتخاب تاریخ")}
          </p>

          {years > 0 && <p className="!text-main"> {years}سال</p>}
          {!alert && (
