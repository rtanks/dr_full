        ...
        <div onClick={close} className="w-full h-full flex justify-center absolute left-0 top-0 bg-[#0009]">
            ...
            <div onClick={(e) => e.stopPropagation()} className="w-[96%] sm:w-[30%] h-max mt-10 p-2 sm:p-5 sm:items-center rounded-xl bg-white flex flex-col pb-1">
                ...
                <div className="w-full h-max flex flex-col gap-5 pb-4 px-2">
                    <div className="w-full h-max font-bold text-lg">درخواست {historyData?.request?.service}</div>
                    ...
                    <div className="w-full h-max flex flex-row justify-between items-center gap-1">
                    ...
                    <div className="w-full h-max flex flex-col gap-1">
                        ...
                        <div className="w-full h-max text-898989 flex flex-row gap-1 px-1">{historyData?.request?.area.map((item,index) => index == 0? <span>{item}</span>: <span>, {item}</span>)}</div>
                    </div>
                    <div className="w-full h-max flex flex-col gap-0.5">
                        <div className="font-bold">توضیحات:</div>
                        <p className="w-full h-max text-898989 rounded-xl p-1">{historyData?.request?.explain}</p>
                    </div>
                </div>
            </div>
        </div>
