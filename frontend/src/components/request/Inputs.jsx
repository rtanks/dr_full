...
export function InputWithLabel({type, label, mode, placeholder, register, hasError, onClick, isValid, maxLength}){
    return(
        <>
            <label className="w-full h-max text-unselect text-md pr-0.5">
                {label}
            </label>
            <motion.input inputMode={mode} maxLength={maxLength} onClick={onClick}
