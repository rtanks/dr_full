...
export default function Failed() {
    ...
    const {data, isLoading, isSuccess, isPending} = useQuery({queryKey: ['transactionResult'], queryFn: async () => {
        const response = await axios.get(`${baseUrl}/payment/payment-inquiry/${id}`, {headers});
    ...
    return (
        <div className="w-full h-full flex justify-center">
            <div className='w-full sm:w-[402px] mt-[10%] h-max py-[7%] px-[5%] flex flex-col gap-2 items-center'>
                ...
                <div className='w-full pt-3 text-center h-12 text-red-600 mb-4'>تراکنش ناموفق</div>
                ...
                <button className='w-full bg-red-600 text-white py-2 rounded-xl'>بازگشت</button>
            </div>
        </div>
    )
}