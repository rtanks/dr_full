...
import SearchUserAutoFill from "./patient/SearchUserAutoFill";
    ...
    const onSubmit = (data) => {
        ...
        if(medicineSelected.filter(item => item.name == '').length != 0) {
          ...
          createRequestMutation.mutate({
            ...
            request:{service: 'ویزیت پزشک', medicine: medicineSelected.filter(item => item.name != ''), 
              dateRefer: data.dateRefer, explain: data.explain}});
        }
    }
    useEffect(()=> {
      getValue('phoneNumber', user.phoneNumber);
      getValue('fullName', user.fullName)
    },[user])
    return (
        <>
                          ...
                          <div style={{gridColumn:"1 / -1"}}>
                            <div className="row-three">
                              <div>
                                  ...
                                  <SearchUserAutoFill getValue={getValue} getUser={getUser}/>
                              </div>
                              <div>
                              ...
                              <div>
                                  ...
                                  <input {...register("fullName")} type="text" value={user?.fullName} id="fullName" placeholder="مثال: علی رضایی" />
                              </div>
                              <div>
                                  <SetProvinces setData={getValue} setProvince={setProvince}
                                      province={user?.province || province.name} style={'w-full'}/>
                              </div>
                              <div>
                                  <SetCities setData={getValue} province={province}
                                      style={'w-full'} data={user?.city || watch('city')}/>
                              </div>
                              <div>
                                <Calender
                                  initialDate={user?.birthday || watch('birthday')} placeholder={'تاریخ تولد'}
